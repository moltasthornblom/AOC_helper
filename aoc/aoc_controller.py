...
def get_day_input(day=util.get_diff_date(DATE1, DATE2), year=DATE2.year):
    ...
    if _check_if_valid_day(day):
        print("Invalid day.")
        return 0
    status, response = _aoc_get_request_with_oath(f'http://adventofcode.com/{year}/day/{day}/input')
    if _check_if_status_ok(status):
        return response
    print(f"Aoc responded with code {status}")
    return 0
def post_day_answer(answer, level="1", day=util.get_diff_date(DATE1, DATE2), year=DATE2.year):
    """Post day answer to aoc"""
    uri = f'https://adventofcode.com/{year}/day/{day}/answer'
    response = _aoc_post_request_with_oath(uri,
                                f'level={level}&answer={answer}')[1]
    print(response)
